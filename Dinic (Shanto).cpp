...
{
    BeatMeScanf;
    int i,j,k,n,m,u,v,w,x;
    cin>>n>>m>>x;
    for(i=1;i<=m;i++) cin>>u>>v>>w,flow.addedge(u,v,w);
    vi vec;
    for(i=1;i<=n;i++) for(auto& e:flow.g[i]) vec.eb(e.w);
    double l=0.0,r=1e6;
    int op=200;
    while(op--){
        double mid=(l+r)/2.0;
        for(i=1;i<=n;i++) for(auto& e:flow.g[i]) e.flow=0,e.w=floor(1.0*e.w/mid+eps);
        if(flow.max_flow(1,n)>=x) l=mid;
        else r=mid;
        int k=0;
        for(i=1;i<=n;i++) for(auto& e:flow.g[i]) e.w=vec[k++];
    }
    cout<<fout(10)<<l*x<<nl;
    return 0;
}
